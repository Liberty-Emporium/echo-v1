...
The GitHub PAT `ghp_REDACTED_EXPIRED_TOKEN` **returns 401 on all GitHub REST/GraphQL API calls** but works for `git clone` and `git push`. This token lacks the `repo` scope needed for the GitHub API.
